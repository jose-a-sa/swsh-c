...
real_t BClimYp(int s, int l, int m)
{
    if (l < MAX(abs(s), abs(m)))
...
real_t BClimYm(int s, int l, int m)
{
    if (l < MAX(abs(s), abs(m)))
        GSL_ERROR("\nBClimYm function s,l,m input error.\n", GSL_EDOM);
...
real_t g(int j, vector_t y, real_t x, int s, int l, int m, real_t c)
{
    ...
    {
        result = 2 * x * VECTOR_GET(y, 1) - VECTOR_GET(y, 0) * (powl(c * x, 2.0) - 2 * c * s * x + VECTOR_GET(y, 2) - powl(m + s * x, 2.0) / (1.0 - powl(x, 2.0)));
...
real_t Dg(int j, int i, vector_t y, real_t x, int s, int l, int m, real_t c)
{
    ...
    {
        if(i==0)
            result = -1.0* (powl(c * x, 2.0) - 2 * c * s * x + VECTOR_GET(y, 2) - powl(m + s * x, 2.0) / (1.0 - powl(x, 2.0)));
        else if(i==1)
            result = 2.0 * x;
        else if(i==2)
